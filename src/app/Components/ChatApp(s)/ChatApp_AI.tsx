...
const Prompts = ({ getPrompt }: PromptsProps) => {
  ...
  return (
    loadNewPrompts && (
      <motion.div
        className="absolute bottom-12 w-full flex justify-center gap-2 p-2 z-[10]"
...
const Chatapp_AI = () => {
  ...
  const handleUserPrompt = (prompt: string) => {
    ...
    fetch("/api/chatapp/ai-response", {
      ...
      .then((res) => {
        ...
        // Structure 2, using async await func
        async function readStream() {
          ...
          try {
            while (true) {
              ...
              // When the first chunk receive create a dummy ai msg that will get replaced/updated with latest data(chunks)
              if (receivedText.length === chunk.length) {
                setChatHistory((prev) => [
                  ...prev,
                  {
                    role: "ai",
  ...
  return (
    <section className="w-[min(70vw,650px)] h-[700px] border-2 border-slate-500 p-3 rounded-md flex flex-col items-center justify-between">
      <h2 className="text-black text-xl font-medium">Chatapp x AI</h2>
